...
# NetraSense Project Override

This project uses Taste Skill as a guardrail against generic AI frontend output, **but NetraSense has its own binding design constraints**. Before applying any Taste Skill rule, read and prioritize:

- `DESIGN.md` — the project design system (WCAG 2.2 AAA, touch targets, audio/haptic, color tokens).
- `CONTEXT.md` — domain glossary and accessibility-first terminology.

NetraSense is an **accessibility-critical assistive app** for blind and low-vision users, not a marketing landing page. Taste Skill defaults tuned for landing pages/portfolios must be overridden when they conflict with NetraSense constraints. Non-negotiable overrides:

- **Accessibility beats aesthetics.** Contrast, touch target size (64dp), screen-reader order, and motion safety take precedence over trendy layouts.
- **Silence is the safe signal.** The product intentionally does not announce itself when the path is clear. Decorative motion and ambient animation are banned.
- **Color palette is locked** to NetraSense tokens (Paper, Ink, Amber, Violet, semantic Red/Orange/Green/Teal). Do not introduce new accent colors.
- **Typography follows Atkinson Hyperlegible** when possible, with clear hierarchy and generous tracking for low-vision users.
- **Target sentuh minimum 64dp** (88dp for primary actions). Compact "editorial" spacing is not acceptable here.
- **Motion must respect `prefers-reduced-motion`.** No scroll-jacking, no auto-playing motion, no physics-heavy hover effects.

Apply Taste Skill for layout craft, hierarchy, spacing discipline, and anti-cliché composition — but never let it override NetraSense's accessibility-first design system.
